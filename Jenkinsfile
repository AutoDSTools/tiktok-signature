...
def updateDeploy(appList) {
    appList.each {
        kubectl_command = "kubectl -n ${NAMESPACE} --context=eks set image ${it.value} ${it.key} ${it.key}=${ECR_REPO}:${TAG}"
        sh kubectl_command
            ...
            stage('Build Docker Image') {
                withEnv([
                    ...
                    sh '''
                        ...
                        aws ecr get-login-password --region us-west-2 | docker login --username AWS --password-stdin 956449821269.dkr.ecr.us-west-2.amazonaws.com
                    '''.stripIndent()
                }
            ...
            if (BRANCH_NAME in env.keySet()) {
                stage('Get list of deployments') {
                    withEnv([
                        ...
                        appListStr = sh (
                                returnStdout: true,
                                script: 'kubectl get -n ${NAMESPACE} --context=eks -o custom-columns=NAME:.metadata.name deployment | grep \'^tiktok-signature\' | awk \'{ quote = "\'\\\'\'"; finish = quote":"quote"deploy"quote; deployStr = quote $1 finish ; print deployStr}\' '
                        ).trim()
                    }
                ...
                stage('Check status'){
                    appList.each {
                        kubectl_check = sh(returnStatus: true, script: "kubectl --context=eks rollout status -n ${NAMESPACE} ${it.value}/${it.key} --timeout=20m" )
                        if ( kubectl_check != 0 ){
                            appList.each {
                                revision = sh(returnStdout: true, script: "kubectl -n ${NAMESPACE} --context=eks rollout history ${it.value}/${it.key}| grep -E '^[0-9]' | tail -n3 | head -n1 | awk {'print\$1'}" )
                                kubectl_command = "kubectl -n ${NAMESPACE} --context=eks rollout undo ${it.value}/${it.key} --to-revision ${revision}"
                                sh kubectl_command
                            }
